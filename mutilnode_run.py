...
parser.add_argument('--world_size', type=str, default='')
...
cmd = 'torchrun --nproc_per_node={} --nnodes={} --node_rank={} --master_addr={} --master_port={} basicsr/train.py -opt {} --launcher {}'.format(args.num_gpus,args.num_nodes,args.rank,master_ip,master_port,args.config,'pytorch')
print('[Main.py]', cmd)
os.system("python basicsr/setup.py develop")
#os.system("pip install -e basicsr/IQA/CLIP-IQA-2-3.8")
os.system(cmd)
os.system('df -h')
print('=================Finished but not stop=======================')
#monitor.stop()