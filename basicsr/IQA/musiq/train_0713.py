...
config = Config({
    # device
    'gpu_id': "2",                          # specify GPU number to use
    'num_workers': 8,

    # data
    'db_name': 'GFIQA-20k',                                     # database type
    'db_path': '/Disk/Data/GFIQA-20k/image/',      # root path of database
    'txt_file_name': '/Disk/Data/GFIQA-20k/mos_val_rating.csv',                # list of images in the database
    'train_size': 0.8,                                          # train/vaildation separation ratio
    'scenes': list(range(19998)),                                            # using all scenes
    'scale_1': 384,                                             
    'scale_2': 224,
    'batch_size': 8,
    'patch_size': 32,
...
elif config.db_name == 'GFIQA-20k':
    from data.GFIQA import IQADataset
